<template>
  <div class="container" style="max-width:720px; padding-top:32px;">
    <SurveyProgressBar current-step="E" />
    <div class="card" v-if="stepData">
      ...
      <p v-if="!canProceed" class="validation-hint">* Harap isi semua pernyataan dan pertanyaan yang wajib sebelum mengirim.</p>
...
onMounted(async () => {
  ...
  try {
    const { data } = await publicApi.getSurveyItems(survey.responseId, 'E')
...
async function saveAndNext() {
  ...
  loading.value = true
  try {
    const items = survey.getAnswersForStep('E')
    const open_answers = survey.getOpenAnswersForSubmit()
    await publicApi.saveAnswers(survey.responseId, { items, open_answers })
    const { data } = await publicApi.submitSurvey(survey.responseId)
    survey.anonymousCode = data.kode_anonim
    router.push({ name: 'survey-thankyou' })
  } catch (e: any) {
    ui.showToast(e.response?.data?.detail || t('common.error'), 'error')
  } finally {
    loading.value = false
